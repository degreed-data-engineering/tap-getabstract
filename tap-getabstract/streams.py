...
class HumanResources(TapWorkdayStream):
    ...
    path = (
        "/ccx/service/degreed_dpt1/Human_Resources/v39.2"  # API endpoint after base_url
    )
    # primary_keys = ["id"]
                        ...
                        th.Property(
                            ...
                            th.ObjectType(
                                th.Property(
                                    ...
                                    th.ObjectType(
                                        th.Property(
                                            "@wd_Formatted_Name", th.StringType
                                        ),
                                        th.Property(
                                            "@wd_Reporting_Name", th.StringType
                                        ),
                                    ),
                                )
                            ),
    ...
    def replace_key_names(self, obj):
        if isinstance(obj, dict):
            return {
                key.replace(":", "_"): self.replace_key_names(val)
    ...
    def parse_response(self, response: requests.Response) -> Iterable[dict]:
        ...
        self.total_pages = int(
            new_json_obj["env_Envelope"]["env_Body"]["wd_Get_Workers_Response"][
                "wd_Response_Results"
            ]["wd_Total_Pages"]
        )
